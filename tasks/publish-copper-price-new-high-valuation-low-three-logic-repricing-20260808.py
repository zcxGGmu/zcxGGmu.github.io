...
DATE = "2026-08-08"
BASE_DT = datetime(2026, 8, 8, 1, 50, 0, tzinfo=timezone(timedelta(hours=8)))
SLUG = "copper-price-new-high-valuation-low-three-logic-repricing"
URL_PATH = f"/2026/{SLUG}/"
FULL_URL = SITE + URL_PATH
TITLE = "铜价新高、估值新低：铜板块三重逻辑共振下的重估窗口"
DESC = "铜矿供应负增长、美国关税预期下的库存搬运、国内旺季和低库存共同推动铜板块进入估值再平衡阶段。"
CATEGORY = "投资"
SERIES = "有色金属"
TAGS = ["铜", "铜价", "铜矿", "铜板块", "供给", "库存", "美国关税", "有色金属", "西部矿业", "铜陵有色", "估值修复", "硫酸硫磺"]
MINUTES = 29
SOURCE_ID = "BV19Vub6bEcG"
PREV_EXISTING_URL = "/2026/sk-hynix-ai-memory-supercycle-target-price/"
PREV_EXISTING_TITLE = "SK海力士进入 AI 存储超级周期：275 万韩元目标价的逻辑"

TASKS = Path(__file__).resolve().parent
BODY_FILE = TASKS / f"{SLUG}-body.html"
DRAFT_ARTICLE = TASKS.parent / "blog-site" / "blog" / "2026" / SLUG / "index.html"
SCRIPT_NAME = Path(__file__).name
MANIFEST_NAME = "publish-copper-price-new-high-valuation-low-three-logic-repricing-20260808-changed-files.json"
...
def cover_svg() -> str:
    c1, c2, c3 = "#2f1b12", "#8b3f16", "#f59e0b"
    title_lines = TITLE.replace("：", "：\n", 1).split("\n")
    title_svg = "".join(
        f'<text x="96" y="{150 + i * 72}" fill="#fff7ed" font-family="Noto Sans SC, PingFang SC, Arial" font-size="{58 if i == 0 else 50}" font-weight="800">{esc(line)}</text>'
  ...
  <g opacity="0.16" stroke="#ffedd5" stroke-width="3">
  ...
  <g filter="url(#shadow)">
    <path d="M150 675 C310 610 455 625 610 505 C780 370 930 455 1090 320 C1245 188 1385 215 1505 128" fill="none" stroke="#fff7ed" stroke-width="15" stroke-linecap="round" opacity="0.88"/>
    <circle cx="610" cy="505" r="40" fill="#fed7aa"/><circle cx="1090" cy="320" r="48" fill="#ffedd5" opacity="0.96"/>
    <rect x="102" y="590" width="500" height="122" rx="24" fill="#fff7ed" opacity="0.95"/>
    <text x="142" y="665" fill="#7c2d12" font-family="Noto Sans SC, PingFang SC, Arial" font-size="42" font-weight="800">16.4 倍 PE</text>
    <text x="142" y="705" fill="#9a3412" font-family="Noto Sans SC, PingFang SC, Arial" font-size="25" font-weight="700">近十年约 27% 分位</text>
  </g>
  <g opacity="0.92">
    <rect x="930" y="610" width="410" height="94" rx="20" fill="#451a03" opacity="0.66"/>
    <text x="964" y="670" fill="#ffedd5" font-family="Noto Sans SC, PingFang SC, Arial" font-size="34" font-weight="800">供给 · 库存 · 旺季</text>
  </g>
  {title_svg}
  <text x="100" y="316" fill="#ffedd5" opacity="0.95" font-family="Noto Sans SC, PingFang SC, Arial" font-size="35" font-weight="700">铜矿负增长 · 美国关税 · 非美低库存</text>
  <text x="102" y="382" fill="#fed7aa" font-family="Noto Sans SC, PingFang SC, Arial" font-size="28" font-weight="600">{esc(DESC[:46])}</text>
</svg>'''
...
def validate(outputs: dict[str, str]) -> None:
    ...
    required = ["铜价", "铜矿", "供给", "需求", "美国关税", "库存", "旺季", "估值", "16.4 倍", "27%", "9-12 倍", "西部矿业", "铜陵有色", "硫酸", "硫磺", "玉龙铜业", "估值修复"]
...
def write_outputs(outputs: dict[str, str]) -> None:
    out_dir = Path("/tmp/copper-price-new-high-valuation-low-three-logic-repricing-publish-output")
...
def create_commit(outputs: dict[str, str], ref: RemoteRef) -> str:
    ...
    commit = run_gh(
        ...
        {
            "message": "Publish copper valuation repricing article",
            "tree": tree["sha"],
